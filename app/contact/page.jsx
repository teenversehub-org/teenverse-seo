...
export const metadata = buildMetadata({
  title: "Contact TeenVerseHub | Support, Safety, Partnerships, and Hiring Questions",
  description: "Contact TeenVerseHub for teen account support, guardian questions, safety concerns, verification, payments, partnerships, and startup hiring inquiries.",
  path: "/contact",
  keywords: [
...
const pageSchema = {
  ...
  '@type': "ContactPage",
  headline: "Contact TeenVerseHub | Support, Safety, Partnerships, and Hiring Questions",
  name: "Contact TeenVerseHub | Support, Safety, Partnerships, and Hiring Questions",
  description: "Contact TeenVerseHub for teen account support, guardian questions, safety concerns, verification, payments, partnerships, and startup hiring inquiries.",
    ...
    <>
      ...
      <main className="min-h-screen bg-[#f7f9ff] text-slate-950 selection:bg-indigo-200 selection:text-slate-950 transition-colors duration-500 dark:bg-[#030712] dark:text-white dark:selection:bg-indigo-500/30 dark:selection:text-indigo-200">
        <section className="relative isolate flex min-h-[90vh] flex-col items-center justify-center overflow-hidden px-6 pb-20 pt-32">
          ...
          <div className="tv-reveal relative z-10 mx-auto max-w-5xl text-center">
            ...
            <p className="mx-auto mt-8 max-w-3xl text-lg leading-relaxed text-slate-600 dark:text-slate-400 md:text-xl">
              {"TeenVerseHub support is built for teenagers, guardians, startups, creators, and partners who need clear answers about accounts, safety, verification, payments, hiring, and the platform vision."}
            </p>
            <div className="mt-12 flex w-full flex-col items-center justify-center gap-4 sm:flex-row sm:gap-6">
              ...
              <a href={"/faq"} className="inline-flex w-full max-w-sm items-center justify-center gap-3 rounded-full border border-indigo-200 bg-white/70 px-8 py-4 text-base font-bold text-slate-900 shadow-sm backdrop-blur-lg transition-colors hover:bg-white dark:border-white/20 dark:bg-white/5 dark:text-white dark:hover:bg-white/10 dark:hover:text-white sm:w-auto">
          ...
          <div className="mx-auto max-w-7xl">
            <div className="tv-reveal max-w-3xl">
              <h2 className="text-4xl font-black tracking-tight text-slate-950 dark:text-white sm:text-5xl">
                {"Get help with the right TeenVerseHub team."}
              </h2>
              <p className="mt-6 text-xl leading-relaxed text-slate-600 dark:text-slate-400">
                {"Whether the question is about a teen account, guardian consent, safety concern, hiring, verification, payout, or partnership, the support path should be clear and responsible."}
              </p>
            </div>
          ...
          <div className="mx-auto max-w-7xl">
            <div className="grid grid-cols-1 gap-16 lg:grid-cols-12 lg:gap-24">
              <div className="tv-reveal lg:sticky lg:top-32 lg:col-span-5 lg:h-max">
                <h2 className="text-4xl font-black leading-tight tracking-tight text-slate-950 dark:text-white sm:text-5xl">{"Support should understand teens, guardians, and clients."}</h2>
                <p className="mt-6 text-lg leading-relaxed text-slate-600 dark:text-slate-400">{"A teen economic ecosystem needs support for many questions: onboarding, guardian consent, verification, account safety, service categories, payments, partnerships, and startup hiring."}</p>
              </div>
              <div className="flex flex-col gap-8 lg:col-span-7">
                <div className="tv-card-motion tv-reveal relative rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-indigo-50 to-white p-7 shadow-2xl shadow-indigo-100/70 backdrop-blur-xl dark:border-indigo-500/20 dark:bg-gradient-to-br dark:from-indigo-950/40 dark:to-slate-900/40 dark:shadow-2xl sm:rounded-[2.5rem] sm:p-10">
                  <div className="mb-6 flex items-center gap-4 border-b border-slate-200 pb-6 dark:border-white/10"><Briefcase className="h-8 w-8 text-indigo-400" /><h3 className="text-2xl font-black text-slate-950 dark:text-white">{"Support Topics"}</h3></div>
                  <ul className="space-y-4">{[
                    "Teen account and profile questions",
                    "Guardian consent and safety questions",
                    "Startup hiring and partnership inquiries",
                    "Payments, verification, and reporting support"
                ...
                <div className="tv-card-motion tv-reveal relative rounded-[2rem] border border-emerald-100 bg-gradient-to-br from-emerald-50 to-white p-7 shadow-2xl shadow-emerald-100/70 backdrop-blur-xl dark:border-emerald-500/20 dark:bg-gradient-to-br dark:from-emerald-950/40 dark:to-slate-900/40 dark:shadow-2xl sm:rounded-[2.5rem] sm:p-10">
                  <div className="mb-6 flex items-center gap-4 border-b border-slate-200 pb-6 dark:border-white/10"><TrendingUp className="h-8 w-8 text-emerald-400" /><h3 className="text-2xl font-black text-slate-950 dark:text-white">{"Responsible Response Paths"}</h3></div>
                  <p className="mb-6 text-lg text-slate-700 dark:text-slate-300">{"The goal is to route urgent safety issues, guardian questions, account support, and business inquiries to clearer next steps."}</p>
                  <ul className="space-y-4">{[
                    "Safety concerns need fast review",
                    "Guardian questions need clear explanations",
                    "Hiring questions need service context",
                    "Partnerships need platform vision details"
        ...
        <section className="relative isolate mx-auto my-24 max-w-[1300px] overflow-hidden rounded-[3rem] border border-white/10 bg-[#090e1a] px-6 py-24 shadow-[0_40px_100px_rgba(0,0,0,0.6)] sm:my-32 sm:px-16 sm:py-32">
          ...
          <div className="tv-reveal relative z-10 mx-auto max-w-3xl text-center"><h2 className="text-4xl font-black tracking-tight text-white sm:text-5xl">{"How to reach the right support path."}</h2><p className="mt-6 text-xl leading-relaxed text-slate-400">{"Clear categories make support faster and more useful."}</p></div>
          ...
          <div className="tv-reveal relative z-10 mt-12 rounded-[2rem] border border-rose-500/20 bg-rose-950/20 p-10 backdrop-blur-md"><div className="flex flex-col items-start gap-8 sm:flex-row sm:items-center"><div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-rose-500/20 text-rose-400"><AlertTriangle className="h-8 w-8" /></div><div><h4 className="text-2xl font-black text-white">{"Support Note"}</h4><p className="mt-2 text-lg text-slate-400">{"For urgent safety concerns, account risk, payment disputes, or guardian questions, use clear written details so the support team can review responsibly."}</p></div></div></div>
        </section>

        <section className="relative overflow-hidden bg-gradient-to-t from-[#0f172a] to-[#030712] px-6 py-24 text-center sm:py-32">
          <div className="tv-reveal relative z-10 mx-auto max-w-4xl"><h2 className="text-4xl font-black tracking-tight text-white sm:text-6xl">{"Contact TeenVerseHub with clear context."}</h2><p className="mx-auto mt-8 max-w-2xl text-xl leading-relaxed text-slate-400">{"Reach out for support, safety review, platform questions, hiring interest, verification help, payment questions, or partnership conversations."}</p><div className="mt-12 flex flex-col items-center justify-center gap-6 sm:flex-row"><a href={`mailto:${SITE.supportEmail}`} className="inline-flex w-full items-center justify-center gap-3 rounded-full bg-indigo-500 px-10 py-5 text-lg font-black text-white transition-all hover:bg-indigo-400 hover:shadow-[0_0_40px_rgba(99,102,241,0.4)] active:scale-95 sm:w-auto">{"Email Support"}</a><a href={"/faq"} className="inline-flex w-full items-center justify-center gap-3 rounded-full border border-white/20 bg-white/5 px-10 py-5 text-lg font-bold text-white backdrop-blur-lg transition-colors hover:bg-white/10 sm:w-auto">{"Read FAQ"}</a></div></div>
        </section>
      </main>
