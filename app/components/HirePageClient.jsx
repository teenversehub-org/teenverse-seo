...
const HirePageClient = ({ initialDarkMode = true }) => {
  ...
  const handleNav = (target) => {
    ...
    // External App Routes (To the Dashboard)
    const externalRouteMap = {
        ...
        'post a project': 'https://app.teenversehub.in/login',
  ...
  return (
    <div className={`relative min-h-screen selection:bg-[#ccff00] selection:text-black transition-[background-color,color] duration-500 ease-out ${darkMode ? 'bg-[#050505] text-white' : 'bg-[#f8f9ff] text-slate-900'}`}>
      ...
      <div className={`fixed top-[-10%] left-[-10%] w-[50vw] h-[50vw] rounded-full blur-[150px] pointer-events-none transition-opacity duration-700 transform-gpu will-change-transform ${darkMode ? 'bg-[#ccff00]/10' : 'bg-indigo-400/20'}`} />
      ...
      <nav className="animate-slide-down fixed w-full z-50 top-0 py-4 px-6">
        <div className={`max-w-7xl mx-auto rounded-2xl px-6 py-3 flex justify-between items-center shadow-2xl relative z-50 transition-all ${darkMode ? 'bg-black/80 backdrop-blur-xl border border-white/10' : 'bg-white/80 backdrop-blur-xl border border-indigo-100 shadow-indigo-100/50'}`}>
           ...
           <div className="hidden md:flex items-center gap-8 text-sm font-bold uppercase tracking-wider">
              ...
              <button onClick={() => handleNav('post a project')} className={`px-6 py-2 rounded-xl transition-all hover:scale-105 font-black hover-target ${darkMode ? 'bg-[#ccff00] text-black hover:bg-yellow-400' : 'bg-indigo-600 text-white hover:shadow-lg hover:shadow-indigo-200 hover:bg-indigo-700'}`}>
                POST A PROJECT
              </button>
           </div>
          ...
          {isMobileMenuOpen && (
            ...
            >
              <div className={`border rounded-2xl p-6 flex flex-col gap-6 shadow-2xl backdrop-blur-2xl ${darkMode ? 'bg-[#0a0a0a] border-white/15' : 'bg-white border-indigo-100'}`}>
                 ...
                 <button onClick={() => handleNav('post a project')} className={`w-full py-4 rounded-xl font-black uppercase tracking-wider flex items-center justify-center gap-2 shadow-lg ${darkMode ? 'bg-[#ccff00] text-black' : 'bg-indigo-600 text-white'}`}>
                   Post a Project <Rocket size={18} />
                 </button>
              </div>
            </motion.div>
          )}
      ...
      {/* --- 🎯 HERO SECTION (SEO Optimized for "Hire") --- */}
      <section className="relative pt-48 pb-20 px-6 overflow-hidden text-center">
         <div className="max-w-5xl mx-auto relative z-10">
            <div className={`animate-fade-up delay-100 inline-flex items-center gap-2 px-4 py-2 rounded-full border text-xs font-mono font-bold tracking-widest mb-8 ${darkMode ? 'border-[#ccff00]/30 bg-[#ccff00]/10 text-[#ccff00]' : 'border-indigo-200 bg-indigo-50 text-indigo-700'}`}>
              <Zap size={14} /> FOR STARTUPS, CREATORS & BUSINESSES
            </div>
            ...
            <h1 className={`animate-fade-up delay-200 text-5xl md:text-7xl lg:text-[5.5rem] font-black leading-[1.05] tracking-tighter mb-8 ${darkMode ? 'text-white' : 'text-slate-900'}`}>
              Hire Skilled Teen <br className="hidden md:block"/>
              <span className={`text-transparent bg-clip-text ${darkMode ? 'bg-gradient-to-r from-[#ccff00] to-green-400' : 'bg-gradient-to-r from-indigo-600 to-purple-500'}`}>Freelancers.</span>
            </h1>
            ...
            <p className={`animate-fade-up delay-300 text-xl font-medium leading-relaxed max-w-2xl mx-auto mb-12 ${darkMode ? 'text-gray-400' : 'text-slate-600'}`}>
              Outsource your tasks to verified, ambitious student talent. Get affordable video editing, web development, design, and more—delivered with Gen-Z creativity.
            </p>

            <div className="animate-fade-up delay-400 flex flex-col sm:flex-row gap-4 justify-center items-center">
               <button 
                 onClick={() => handleNav('post a project')}
      ...
      <section className={`py-10 border-y ${darkMode ? 'bg-[#080808] border-white/5' : 'bg-slate-50 border-slate-200'}`}>
         <div className="max-w-7xl mx-auto px-6 flex flex-wrap justify-center gap-8 md:gap-16 text-sm font-bold uppercase tracking-wider">
            <span className={`flex items-center gap-2 ${darkMode ? 'text-gray-400' : 'text-slate-600'}`}>
              <ShieldCheck size={20} className={darkMode ? 'text-[#ccff00]' : 'text-indigo-500'} /> Verified Students
            </span>
            <span className={`flex items-center gap-2 ${darkMode ? 'text-gray-400' : 'text-slate-600'}`}>
              <Lock size={20} className={darkMode ? 'text-[#ccff00]' : 'text-indigo-500'} /> Escrow Payments
            </span>
            <span className={`flex items-center gap-2 ${darkMode ? 'text-gray-400' : 'text-slate-600'}`}>
              <Users size={20} className={darkMode ? 'text-[#ccff00]' : 'text-indigo-500'} /> Guardian Consent
            </span>
         </div>
      </section>

      {/* --- 💡 WHY HIRE TEENS (The USP) --- */}
          ...
          <div className="md:w-1/2">
            <RevealOnScroll>
              <h2 className={`text-4xl md:text-5xl font-black mb-6 ${darkMode ? 'text-white' : 'text-slate-900'}`}>
                Why hire student freelancers?
              </h2>
              <p className={`text-lg mb-8 leading-relaxed ${darkMode ? 'text-gray-400' : 'text-slate-600'}`}>
                Stop overpaying agencies for tasks that don't require decades of experience. Teenagers today are digital natives—they've been editing videos and writing code since they were 12.
              </p>
              <div className="space-y-6">
                {[
                  { title: "Highly Affordable", desc: "Get professional-level output without the agency price tag." },
                  { title: "Gen-Z Creativity", desc: "They inherently understand modern social media trends and algorithms." },
                  { title: "Eager to Prove Themselves", desc: "They aren't just working for money; they are building their first real portfolio." }
                ].map((item, i) => (
             ...
             <RevealOnScroll delay={0.2}>
               <div className={`aspect-square rounded-[3rem] border relative overflow-hidden flex items-center justify-center ${darkMode ? 'bg-[#111] border-white/10' : 'bg-indigo-50 border-indigo-100'}`}>
                  ...
                  <div className="text-center relative z-10 p-8">
                     <Target size={64} className={`mx-auto mb-6 ${darkMode ? 'text-[#ccff00]' : 'text-indigo-600'}`} />
                     <h3 className={`text-3xl font-black mb-2 ${darkMode ? 'text-white' : 'text-slate-900'}`}>Fast. Affordable. Driven.</h3>
                     <p className={darkMode ? 'text-gray-400' : 'text-slate-600'}>The perfect talent pool for startups.</p>
                  </div>
               </div>
             </RevealOnScroll>
          </div>
          ...
          <RevealOnScroll>
            <div className="text-center mb-16">
              <h2 className={`text-4xl md:text-5xl font-black mb-4 ${darkMode ? 'text-white' : 'text-slate-900'}`}>
                What can you <span className={darkMode ? 'text-[#ccff00]' : 'text-indigo-600'}>outsource?</span>
              </h2>
            </div>
          </RevealOnScroll>
      ...
      <section id="how-it-works" className={`py-24 px-6 border-t ${darkMode ? 'bg-[#050505] border-white/5' : 'bg-white border-slate-200'}`}>
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            {[
              { num: "01", title: "Post a Job", desc: "Describe what you need done and set your budget." },
              { num: "02", title: "Review Pitches", desc: "Verified teens apply with their portfolios and ideas." },
              { num: "03", title: "Fund Escrow", desc: "Your payment is held securely until the work is approved." },
              { num: "04", title: "Get Results", desc: "Review the work, approve it, and the funds are released." }
            ].map((step, i) => (
        ...
        <RevealOnScroll>
          <div className={`max-w-5xl mx-auto p-10 md:p-20 rounded-[3rem] text-center relative overflow-hidden ${darkMode ? 'bg-[#111] border border-white/10' : 'bg-indigo-600 text-white shadow-2xl shadow-indigo-300'}`}>
            <div className="relative z-10">
              <h2 className="text-4xl md:text-6xl font-black mb-6 text-white">Ready to hire?</h2>
              <p className={`text-lg md:text-2xl mb-10 max-w-2xl mx-auto font-medium ${darkMode ? 'text-gray-400' : 'text-indigo-100'}`}>
                Post your first project today and tap into the next generation of digital talent.
              </p>
              <button 
                onClick={() => handleNav('post a project')} 
              ...
              >
                Post a Project Now <ArrowRight size={20} />
              </button>
            </div>
          </div>
        </RevealOnScroll>
